...
# Get associated CUs from crosswalkList for those SMUs
excluded_cus = crosswalkList %>%
  filter(smu %in% smu_rows$Name) %>%
  pull(cu) %>%
  unique()

# Step 3: Get reference CUs from crosswalkList, excluding those tied to SMUs
